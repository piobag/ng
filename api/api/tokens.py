...
from flask import (
    Blueprint, request, abort, current_app, url_for, session, redirect
)
...
from api.decorators import get_json
...
def refresh(data):
    ...
    refresh_token = data.get('refresh_token', request.cookies.get(
        'refresh_token'))
    ...
    ntoken = new_token.dict()
    del ntoken['id']
    db.token.insert_one(ntoken)
...
def reset(args):
    ...
    if user is not None:
        ...
        reset_url = current_app.config['NEXT_APP_URL'] + \
            '/reset?token=' + reset_token
        # Dev user passthrough
        if u.email in current_app.config['MAIL_ADMIN'] or \
                    u.email in current_app.config['MAIL_USERS']:
            return redirect(reset_url)
        send_email(u.email, 'Altere sua senha', 'reset',
                   username=u.name, token=reset_token, url=reset_url)
    return {}, 204
...
def password_reset(args):
    ...
    if user is None:
        abort(400)
    u = User(**user)
    u.set_password(args['new_password'])
    db.user.update_one({'email': u.email}, {'$set': {'password': u.password}})
    return {}, 204
